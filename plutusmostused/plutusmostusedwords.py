...
def generate_lines():
	...
	linea = open('mostusedeng10000.txt', "r").read().splitlines() #as f:
	#g = open('lastname 88000.txt', "r").read().splitlines()  # as f:
	#h = open('cities 79000.txt', "r").read().splitlines()  # as f:
	# passphrase = f.readlines()
	#lines = f.read().splitlines()
	linesa = random.choice(linea)
	linesb = random.choice(linea)
	linesc = random.choice(linea)
	linesd = random.choice(linea)
	#linese = random.choice(f)
	#return (linesa.strip()+" "+linesb.strip())
	return (linesa.strip()+" "+linesb.strip()+" "+linesc.strip()+" "+linesd.strip())
	#passphrase = (linesa.strip()+linesb.strip()+linesc.strip()+linesd.strip()+linese.strip())
...
def generate_private_key(passphrase):
	...
	return hashlib.sha256(passphrase.encode("utf-8")).hexdigest()
		#return hashlib.sha256(passphrase('utf-8')).hexdigest()
	#return binascii.hexlify("txt").decode('utf-8').upper()
...
def process(passphrase, private_key, public_key, address, database):
	"""
...
def main(database):
	...
	"""
	while True:
		passphrase = generate_lines()
		private_key = generate_private_key(passphrase)			# 0.0000061659 seconds
		...
		process(passphrase, private_key, public_key, address, database) 	# 0.0000026941 seconds
									# --------------------
